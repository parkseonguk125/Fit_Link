...
import { CARDIO_TYPE_OPTIONS, cardioTypeLabel } from "@/lib/constants";

const PLACEHOLDER = "선택하세요 (러닝, 사이클 등)";
const VISIBLE_ITEM_COUNT = 5;
const ITEM_HEIGHT_PX = 48;

type CardioTypeSelectProps = {
  value: CardioType | "";
  onChange: (value: CardioType | "") => void;
};

export function CardioTypeSelect({ value, onChange }: CardioTypeSelectProps) {
  ...
  function selectOption(next: CardioType | "") {
    onChange(next);
    setOpen(false);
  }

  const selectedLabel = value ? cardioTypeLabel(value) : PLACEHOLDER;
